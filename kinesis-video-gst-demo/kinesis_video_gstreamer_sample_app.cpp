...
static GstFlowReturn on_new_sample(GstElement *sink, CustomData *data) {
    ...
    if (!isDroppable) {
        ...
        if (!(isHeader && (!GST_BUFFER_PTS_IS_VALID(buffer) || !GST_BUFFER_DTS_IS_VALID(buffer)))) {
            ...
            if (false ==
                put_frame(data->kinesis_video_stream, frame_data, buffer_size, std::chrono::nanoseconds(buffer->pts),
...
static bool resolution_supported(GstCaps *src_caps, gchar *filter, int width, int height, int framerate) {
    ...
    gchar *s = g_strdup_printf("%s, width=(int)%i, height=(int)%i, framerate=(fraction)%i/1", filter, width, height,
                               framerate);
    GstCaps *query_caps;
...
int gstreamer_init(char *stream_name, int width = 1920, int height = 1080, int framerate = 24, int bitrate = 1000000) {
    ...
    */
    ...
    gchar *s = g_strdup_printf("%s, width=(int)%i, height=(int)%i, format=(string)%s, framerate=(fraction)%i/1",
                               SOURCE_FILTER, width, height, SOURCE_FORMAT, framerate);
    ...
    g_object_set(G_OBJECT(data.encoder), "control-rate", 1, "qp-range", "-1,-1:-1,-1:-1,-1", "iframeinterval",
                 6/*, "bitrate", bitrate*/, NULL);
    ...
    gst_bin_add_many(GST_BIN(data.pipeline), data.source/*, data.video_convert*/, data.source_filter,
                     data.encoder/*, data.h264parse, data.filter*/, data.appsink, NULL);
    if (gst_element_link_many(data.source/*, data.video_convert*/, data.source_filter,
                              data.encoder/*, data.h264parse, data.filter*/, data.appsink, NULL) != TRUE) {
